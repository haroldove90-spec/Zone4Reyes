...
interface ProfileHeaderProps {
  ...
  onNavigate: (view: 'chat', user: User) => void;
}
...
export const ProfileHeader: React.FC<ProfileHeaderProps> = ({ user, currentUser, isCurrentUser, onUpdateAvatar, onUpdateCover, onEditProfile, onAddFriend, onRemoveFriend, onNavigate }) => {
        ...
        <div className="sm:flex sm:items-end sm:space-x-5">
           ...
           <div className="mt-4 sm:mt-0 flex flex-wrap justify-center sm:justify-end items-center gap-2">
            {isCurrentUser ? (
                ...
                <>
                    {isFriend ? (
                       <button onClick={() => onRemoveFriend(user.id)} className="flex items-center space-x-2 bg-gray-200 text-text-primary font-semibold py-2 px-4 rounded-lg hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 transition-colors">
                            <Icon path="M13 7a4 4 0 11-8 0 4 4 0 018 0zM9 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" className="w-5 h-5"/>
                            <span>Amigos</span>
                        </button>
                    ) : (
                        <button onClick={() => onAddFriend(user.id)} className="flex items-center space-x-2 bg-primary text-white font-semibold py-2 px-4 rounded-lg hover:bg-primary-dark transition-colors">
